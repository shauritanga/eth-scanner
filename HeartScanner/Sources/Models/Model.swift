...
class Model: ObservableObject {
    ...
    func connectProbe(simulated: Bool) async {
        ...
        if simulated {
            ...
            // Physical probe auto-detection; no explicit connect call needed
            // Trigger state update to check compatibility
            imaging.states = { [weak self] state, _ in
                self?.setState(state, imagingStateChanges: ImagingStateChanges(bModeImageChanged: false, mModeImageChanged: false))
            }
        }
        inProgress = false
    }
